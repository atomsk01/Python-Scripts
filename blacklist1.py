...
#This is a basic password checker implementing a whitelist and blacklist
#user may only use characters in the whitelist
#user is prohibited from using characters in the blacklist
...
#banner flair
print("------------------------------")
print("| Blacklist Password Checker |")
print("------------------------------")
...
#will now also check against blacklist
...
print("The following special characters are not allowed # - / ?")
...
while True:
	if (len(password) < 4): #fail if pass shorter than 4
		...
		password = getpass.getpass("Password: ")
	elif re.search(blacklist, password): #input contains blacklist characters
		banana = -1
		prYellow("Password must not contain ? # - /")
		password = getpass.getpass("Password: ")
	else:
